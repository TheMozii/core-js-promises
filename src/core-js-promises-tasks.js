...
function getPromise(number) {
  return new Promise((resolve, reject) => {
    if (number >= 0) {
      ...
      reject(new Error(`Number ${number} is negative`));
    }
  });
}
...
function queuePromises(promises) {
  return promises.reduce(
    (accumulatorPromise, currentPromise) => {
      return accumulatorPromise.then((resultString) => {
        return currentPromise.then((value) => resultString + value.toString());
      });
    },
    new Promise((resolve) => {
      resolve('');
